...
from image_edit_interpreter import ImageEditInterpreter
from image_edit_synthesis import ImageEditSynthesizer
from image_edit_benchmarks import image_edit_benchmarks
    ...
    def get_labelling_qs(self, input_qs):
        ...
        for img, abs_img in input_qs.items():
            for obj_id, obj in abs_img["conf"].items():
                if not obj["Flag"]:
                    label_qs.append((img, obj_id, "Flag"))
                for attr in ATTRIBUTES: 
                    if attr in obj and len(obj[attr]) == 2:
                        label_qs.append((img, obj_id, attr))
        return label_qs
