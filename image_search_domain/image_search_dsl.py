...
class Map(Expression):
    def __init__(
        ...
        '''
        A Map has an expression, a "restriction" and a position. A Map outputs all "restriction" objects
        that have a "position" relationship to some object in "expression". For instance, if the expression is IsObject(Dog),
        the restriction is IsObject(Cat), and the positiion is GetLeft, then the Map will output all Cats that are to the left 
        of a dog.
        '''
...
class GetAround(Position):
    pass 


class GetNotAround(Position):
    pass
